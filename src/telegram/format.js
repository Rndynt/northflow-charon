...
export function formatPosition(position) {
  const pnl = position.pnl_percent != null
    ...
    : position.entry_mcap && position.current_mcap
      ? (Number(position.current_mcap) / Number(position.entry_mcap) - 1) * 100
      : position.entry_mcap && position.high_water_mcap
        ? (Number(position.high_water_mcap) / Number(position.entry_mcap) - 1) * 100
        : 0;
  const staleness = position.status === 'open' && position.last_checked_at_ms
    ? Math.round((Date.now() - Number(position.last_checked_at_ms)) / 1000)
    : null;
  return [
    ...
    position.status === 'open' && position.current_mcap
      ? `Entry mcap: ${fmtUsd(position.entry_mcap)} · Current: ${fmtUsd(position.current_mcap)} · High: ${fmtUsd(position.high_water_mcap)}`
